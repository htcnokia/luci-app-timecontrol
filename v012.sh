...
-- 高级设置
o = s:option(Flag, "auto_fix_apple", translate("Auto fix Apple devices to static IP"))
o.default = 0
o.rmempty = false

o = s:option(Value, "ip_start", translate("Apple IP Range Start"))
o.default = "192.168.188.100"

o = s:option(Value, "ip_end", translate("Apple IP Range End"))
o.default = "192.168.188.150"
...
scan_apple() {
    local auto_fix=$(uci_get_by_type basic auto_fix_apple 0)
    [ "$auto_fix" != "1" ] && return 0

    ubus call dhcp ipv4leases | jsonfilter -e "@.device[@.mac]{mac,ip,hostname,clientid,vendorid}" | while read line; do
        local mac=$(echo $line | awk '{print $1}')
        local ip=$(echo $line | awk '{print $2}')
        local hostname=$(echo $line | awk '{print $3}')
        local clientid=$(echo $line | awk '{print $4}')
        local vendorid=$(echo $line | awk '{print $5}')

        if is_apple_device "$hostname" "$vendorid" "$clientid"; then
            add_static_lease "$mac" "$hostname"
        fi
    done
}
...
is_apple_device() {
    ...
    # Hostname 特征
    echo "$hostname" | grep -qiE "iphone|ipad|macbook|imac|apple" && return 0

    # Vendor Class ID (Option 60)
    echo "$vendorid" | grep -qiE "ios|apple|macos" && return 0

    # Client ID 特征
    echo "$clientid" | grep -qi "apple" && return 0
...
add_static_lease() {
    local mac="$1"
    local hostname="$2"
    local start_ip=$(uci_get_by_type basic ip_start 192.168.188.100)
    local end_ip=$(uci_get_by_type basic ip_end 192.168.188.150)

    # 找一个可用 IP
    local base=$(echo $start_ip | cut -d. -f1-3)
    local start=$(echo $start_ip | cut -d. -f4)
    local end=$(echo $end_ip | cut -d. -f4)

    for i in $(seq $start $end); do
        local ip="$base.$i"
        # 检查是否已被使用
        grep -q "option ip '$ip'" /etc/config/dhcp && continue
        # 添加静态租约
        uci add dhcp host
        ...
        logger -t timecontrol "分配苹果设备 $hostname($mac) 固定IP $ip"
        break
    done
}
...
msgid "Auto fix Apple devices to static IP"
msgstr "處理蘋果設備爲指定IP區段"

msgid "Apple IP Range Start"
msgstr "蘋果設備IP分配起："

msgid "Apple IP Range End"
msgstr "蘋果設備IP分配止："
