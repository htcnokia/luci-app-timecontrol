...
add_rule(){
	...
	for i in $(seq 0 $count); do
		...
		if [ "$enable" == "1" ] && is_mac_address "$macaddr"; then
			...
			# ★ 構建 conntrack 參數（嚴格模式）
			local conntrack_rule=""
			if [ "$strict_mode" == "1" ]; then
				# 嚴格模式：阻止所有流量（包括已建立的連接）
				conntrack_rule=""
			else
				# 寬鬆模式：只阻止新連接
				conntrack_rule="-m conntrack --ctstate NEW"
			fi
			...
			# ★ 嚴格模式額外操作：在時間切換時清除 conntrack
			if [ "$strict_mode" == "1" ]; then
				# 立即斷開現有連接
				conntrack -D -s 0.0.0.0/0 -m $macaddr 2>/dev/null || true
			fi
		fi
	done
...
setup_cron_job() {
	...
	if [ "$strict_mode" == "1" ]; then
		...
		echo "Strict mode enforcement enabled"
	fi
}
...
enforce() {
	...
	for i in $(seq 0 $count); do
		...
		if [ "$timeon_minutes" -ge "$timeoff_minutes" ]; then
			# 跨日
			if [ "$now_minutes" -ge "$timeon_minutes" ] || [ "$now_minutes" -lt "$timeoff_minutes" ]; then
				in_block_time=1
			fi
		else
			# 同日
		...
		# 如果在禁止時間段，清除該 MAC 的所有連接
		if [ "$in_block_time" == "1" ]; then
			conntrack -D -m $macaddr 2>/dev/null || true
		fi
	done
}
